...
import { SealedPositionList } from '../perp/SealedPositionList';
...
export function SpotTerminal() {
    ...
    const handleTrade = async (side: 'long' | 'short') => {
        ...
        const collateralAmount = parseFloat(size) * tradePrice;
    ...
    const executeTrade = async (side: 'long' | 'short', collateralAmount: number, collateralBaseUnits: bigint, sizeL: string, sizeR: string, priceL: string, priceR: string, zkCalldata: string[], commitment: string, nullifier: string) => {
        ...
        try {
            if (orderType === 'market') {
                const perpContractAddress = process.env.NEXT_PUBLIC_SPOT_CONTRACT || process.env.NEXT_PUBLIC_PERP_CONTRACT;
                if (!perpContractAddress) throw new Error("Missing perp contract address");
            ...
            <div className="flex-1 flex flex-col gap-2 min-h-0">
                ...
                <div className="h-72 bg-card/40 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden flex flex-col shrink-0">
                    <div className="flex gap-1 p-2 bg-black/40 border-b border-white/5 shrink-0">
                        ...
                        >
                            Positions <span className="bg-stark-purple px-1.5 py-0.5 rounded text-[10px]">{activeBets.filter(b => b.mode === 'spot').length}</span>
                        </button>
                        <button
                    ...
                    <div className="flex-1 overflow-auto p-4 custom-scrollbar">
                        {activeTab === 'positions' ? (
                            <SealedPositionList tongoPrivKey={tongoPrivKey} filterMode="spot" />
                        ) : activeTab === 'orders' ? (
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-[10px] text-muted-foreground font-mono">
                                    Max: {currentPrice > 0 ? (houseBalance / currentPrice).toFixed(4) : '0.00'} {selectedAsset}
                                </span>
                            </div>
                            ...
                            <div className="space-y-1 mb-4">
                                <input
                                    ...
                                    value={
                                        currentPrice > 0 && size ?
                                            Math.min(100, Math.round((Number(size) / (houseBalance / currentPrice)) * 100)) || 0
                                            : 0
                                    }
                                    onChange={(e) => {
                                        if (currentPrice > 0) {
                                            const percentage = Number(e.target.value);
                                            const maxSize = houseBalance / currentPrice;
                                ...
                                <div className="flex justify-between items-center px-1 text-[9px] text-muted-foreground font-bold tracking-widest">
                                    <span className="hover:text-white cursor-pointer transition-colors" onClick={() => currentPrice > 0 && setSize(((houseBalance * 0.25) / currentPrice).toFixed(4))}>25%</span>
                                    <span className="hover:text-white cursor-pointer transition-colors" onClick={() => currentPrice > 0 && setSize(((houseBalance * 0.5) / currentPrice).toFixed(4))}>50%</span>
                                    <span className="hover:text-white cursor-pointer transition-colors" onClick={() => currentPrice > 0 && setSize(((houseBalance * 0.75) / currentPrice).toFixed(4))}>75%</span>
                                    <span className="hover:text-stark-orange cursor-pointer transition-colors" onClick={() => currentPrice > 0 && setSize((houseBalance / currentPrice).toFixed(4))}>100%</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-2 mt-auto">
                        ...
                        <div className="flex justify-between text-[10px] font-mono text-muted-foreground mb-4">
                            ...
                            <span className="text-white">
                                ~ ${size && currentPrice ? (parseFloat(size) * currentPrice).toFixed(2) : '0.00'}
                            </span>
                        </div>
